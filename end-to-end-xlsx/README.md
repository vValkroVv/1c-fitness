# Выгрузка XLSX из backup Fitness

## Финальный перенос 20–21 сентября 2026

Актуальная инструкция: [единый план переноса](../docs/20260920_final_migration_plan.md).
Из корня репозитория запускать `python3 scripts/run_fitbase_migration.py --help`.
Автономные точки входа: `scripts/prepare_backup.py` (проверка и восстановление,
новая конфигурация) и `scripts/run_pipeline.py` (полная сборка и проверки).

Теперь создаются XLSX и отдельный JPEG-архив действующих клиентов и реактивации
с валидными телефонами. Менеджеры выбираются из общего пула трёх независимо
от клуба. `backup_finish_at` хранит реальный BackupFinishDate; явный
`run.effective_at` или `run.effective_offset_days` задаёт общий срез. Без явного
параметра действует прежний cutoff по backup. Даты самих операций не сдвигаются.
Доказательство полной готовности — `READY.txt` и `reports/delivery_manifest.json`.

Ниже сохранено описание принятой **июньской поставки**. Его датированные команды
и контрольные количества относятся к истории; для нового backup используется
единый план выше.

## Июньская поставка и историческая воспроизводимость

Эта папка собирает семь файлов для импорта в Fitbase из базы 1С Fitness.
Актуальная выгрузка из `Fitnes-30-06-26.bak` построена на едином срезе
`2026-06-30 23:27:03`: шесть обычных XLSX и один отдельный problem4 XLSX.

Долги problem1–3 теперь рассчитываются прямо из регистра взаиморасчётов 1С
`_AccumRg3305` на cutoff backup. Внешний менеджерский XLSX используется только
как контрольный отчёт и не поставляет значения в итоговую выгрузку. В финальном
XLSX заявок реактивация сохраняется как `Реактивация / Закрытые годовые
абонементы`. Актуальный результат:
...
`_InfoRg3060._Fld3064`. Относительно предыдущей принятой поставки изменён только
XLSX заявок и только две подписи в 28 526 строках реактивации; остальные шесть
корневых XLSX сохранены побайтно.

Если вы открыли проект впервые, разбираться во всех номерных скриптах не надо.
Для обычного запуска нужны четыре вещи:
...
3. команда `scripts/run_funnel_label_rename_20260630.sh`;
4. готовые файлы в
   `output/20260630_delivery_funnel_labels_20260820/`.

Первый таргетированный shell-скрипт вызывает основной `run_pipeline.py` и
проверяет, что вне `end_date` ничего не изменилось. Второй пересобирает заявки
штатным алгоритмом из принятого стейджинга и допускает только две согласованные
замены подписей. Внутренние скрипты вручную запускать обычно не нужно.
...
```shell
.venv/bin/python scripts/compare_manager_debt_to_register.py --help
```
...
`sql` в `config/pipeline_register_debts_20260630.yml` и задают пароль через
переменную среды.
...
| Что нужно | Где лежит |
| --- | --- |
| готовые XLSX | `../output/20260630_delivery_funnel_labels_20260820/` |
| проверка замены подписей | `../output/20260630_delivery_funnel_labels_20260820/reports/funnel_label_change_validation.md` |
| структурная проверка | `../output/20260630_delivery_funnel_labels_20260820/reports/structural_validation.md` |
| проверка дат услуг | `../output/20260630_delivery_funnel_labels_20260820/reports/services_validation_report.md` |
| сверка 274 продаж | `../output/20260630_delivery_funnel_labels_20260820/reports/manager_debt_comparison.md` |
...
| лог переименования воронки | `logs/20260630_funnel_labels_20260820/pipeline.log` |
...
Откройте `config/pipeline_register_debts_20260630.yml` и заполните блок `sql`:
...
finance_validate: verdict=PASS
PIPELINE PASS delivery=.../output/20260630_delivery_register_debts
```

Если `PASS` нет, файлы заказчику передавать рано. Сначала откройте
`work/20260630_register_debts/status.json`, затем лог упавшего этапа в
`logs/20260630_register_debts/`.
...
Все 254 договора прежних problem1–3 находятся в основном файле абонементов.
Отдельно исключён только problem4-договор `00000151350`.
