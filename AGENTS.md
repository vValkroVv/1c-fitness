...
По уточнению заказчика от 20.09.2026 фактическое время backup и время актуальности разделены. Реальный `backup_finish_at` всегда брать из `BackupFinishDate` в `RESTORE HEADERONLY`. Без явного переопределения единый `cutoff_at` равен этому времени. Можно явно задать `run.effective_at` (дата и время до секунды) либо `run.effective_offset_days`; для финального переноса 20–21 сентября согласовано +1 сутки с сохранением времени.

Обязательный контракт для каждого прогона:

```text
backup_finish_at = RESTORE HEADERONLY.BackupFinishDate
cutoff_at = explicitly_selected_effective_at OR backup_finish_at
cutoff_date = date(cutoff_at)
date_stamp = YYYYMMDD(cutoff_at)
```

Тот же `cutoff_at` должен применяться ко всем слоям: клиенты, воронки, карты, абонементы, услуги, шаблоны, problem-файлы и фото. Запрещено переносить cutoff от предыдущего backup или задавать отдельные даты для разных слоёв. Пайплайн должен падать с ошибкой при несовпадении дат слоёв. Фактический `backup_finish_at` не подменять выбранным срезом; исходные даты операций не сдвигать.

Актуальный единый план: `docs/20260920_final_migration_plan.md`. Входная команда: `python3 scripts/run_fitbase_migration.py --help`. Июньские поставки ниже — исторические принятые результаты; для новых backup используется текущий план. Менеджеры — общий пул Пеуна Анастасия Ивановна, Пилия Анастасия Артуровна, Ефремова Алена независимо от клуба. Фото — отдельный JPEG-архив действующих и реактивации из фактически передаваемых клиентов, только с валидными телефонами.
...
##текущая финальная поставка 7 xlsx##
...
Единый срез всех XLSX: `2026-06-30 23:27:03`. В финальном файле заявок
используются подписи `Реактивация / Закрытые годовые абонементы`. Остальные
шесть корневых XLSX побайтно совпадают с принятой поставкой после исправления
дат услуг. Воспроизводимый пакет находится в `end-to-end-xlsx/`, итоговый
отчёт — `docs/20260630_funnel_label_rename_20260820.md`.
