...
7. Заберите девять файлов из `output/20260630_delivery_without_active_problems/`.
...
| `membership_export` | Выгрузка 99 959 фактов по абонементам | UTF-16 TSV |
| `membership_xlsx` | Полный импорт абонементов и 114 шаблонов | ещё 2 XLSX |
| `services_sql` | Факты по 51 нужной услуге | `services_import_facts` |
| `services_export` | Выгрузка 50 215 фактов по услугам | UTF-16 TSV |
...
| `delivery` | Удаление 223 спорных договоров из чистого файла | итоговые 6 + 3 XLSX |
...
```text
logs/20260630/pipeline.log
```

Состояние запуска в JSON:

```text
work/20260630/status.json
```

У каждого этапа есть свой файл в `logs/20260630/`. Если процесс остановился,
...
`work/20260630`, `logs/20260630` и папки с готовыми XLSX. Это нужно, чтобы в
