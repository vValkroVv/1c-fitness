...
SELECT N'_Document154' table_name,
...
SUM(CONVERT(bigint,CASE WHEN j.[_IDRRef] IS NOT NULL AND s.[_IDRRef] IS NOT NULL AND (CONVERT(varbinary(max),j.[_Fld1119RRef])<>CONVERT(varbinary(max),s.[_Fld1119RRef]) OR (j.[_Fld1119RRef] IS NULL AND s.[_Fld1119RRef] IS NOT NULL) OR (j.[_Fld1119RRef] IS NOT NULL AND s.[_Fld1119RRef] IS NULL)) THEN 1 ELSE 0 END)) [changed_Fld1119RRef],
SUM(CONVERT(bigint,CASE WHEN j.[_IDRRef] IS NOT NULL AND s.[_IDRRef] IS NOT NULL AND (CONVERT(varbinary(max),j.[_Fld1116RRef])<>CONVERT(varbinary(max),s.[_Fld1116RRef]) OR (j.[_Fld1116RRef] IS NULL AND s.[_Fld1116RRef] IS NOT NULL) OR (j.[_Fld1116RRef] IS NOT NULL AND s.[_Fld1116RRef] IS NULL)) THEN 1 ELSE 0 END)) [changed_Fld1116RRef] FROM [FitnessRestored_20260630_original].dbo.[_Document154] j FULL JOIN [FitnessRestored_20260630_macos].dbo.[_Document154] s ON j.[_IDRRef]=s.[_IDRRef] FOR JSON PATH, INCLUDE_NULL_VALUES;
