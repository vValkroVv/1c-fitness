...
| H | `create_date` | `Дата создания *` | первая продажа клиента; если продажи нет — дата создания карточки, но строку вынести в отдельный отчет |
...
| B | `фио` | ФИО клиента |
| C | `номер пластиковой карты` | активная пластиковая карта из 1C; если карты нет — пусто и строка в отчете |
...
5. `missing_sales_report.csv` — клиенты без первой продажи, но с датой создания карточки; это ненормальная ситуация, решать отдельно.
...
3. Абонемент длится месяц или более: duration_days = end_date - sale_date.
...
```text
subscription.sale_date <= cutoff_date
AND subscription.end_date >= cutoff_date
AND subscription.status IN active_statuses
AND subscription.duration_days >= min_subscription_days
```
...
active_clients:
  ...
  duration_source: "end_date - sale_date"
  active_statuses: []
```
...
2. Если продаж нет, но есть дата создания карточки клиента — использовать дату создания карточки в XLSX и вынести клиента в missing_sales_report.csv.
3. Если нет ни продажи, ни даты создания карточки — пытаться выгрузить строку, но вынести клиента в missing_required_fields.csv.
```
...
3. Пластиковую карту оставить активную.
4. Если карт несколько — выбрать активную и вынести клиента в multiple_cards_report.csv.
```
...
6. Если у клиента несколько карт, выбрать активную карту, а клиента записать в `multiple_cards_report.csv`.
...
12. `create_date` = первая продажа; если первой продажи нет, использована дата создания карточки и клиент попал в `missing_sales_report.csv`.
...
| У клиента несколько пластиковых карт | Выбрать активную карту, вынести клиента в `multiple_cards_report.csv` |
