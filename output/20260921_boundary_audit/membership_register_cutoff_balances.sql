WITH named AS (
 ...
 SELECT subscription_ref,
  SUM(CASE WHEN period_at<='2026-09-21 20:12:12' THEN quantity ELSE 0 END) AS expected_at_cutoff,
  SUM(CASE WHEN period_at<='2026-09-21 20:12:12' AND _RecordKind=0 THEN raw_quantity ELSE 0 END) AS expected_receipt,
  SUM(CASE WHEN period_at<='2026-09-21 20:12:12' AND _RecordKind=1 THEN raw_quantity ELSE 0 END) AS expected_expense,
  SUM(CASE WHEN period_at>'2026-09-21 20:12:12' THEN quantity ELSE 0 END) AS excluded_future_balance,
  SUM(CASE WHEN period_at>'2026-09-21 20:12:12' THEN 1 ELSE 0 END) AS excluded_future_rows,
